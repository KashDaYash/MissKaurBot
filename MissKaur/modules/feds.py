...
import MissKaur.modules.sql.feds_sql as sql
...
from MissKaur.modules.helper_funcs.alternate import send_message
from MissKaur.modules.helper_funcs.chat_status import is_user_admin
from MissKaur.modules.helper_funcs.extraction import (
    extract_unt_fedban,
    extract_user,
    extract_user_fban,
)
from MissKaur.modules.helper_funcs.string_handling import markdown_parser
...
from MissKaur import REPO, DEVELOPER
...
def nothing():
    ...
    if DEVELOPER != axx:
       print("Kid you've changed developer kindly change it back to @TeamMissKaur")
       sys.exit(1)
    else:
       print("Starting")

    if REPO != "TeamMissKaur/MissKaur":
       print("Kid you've changed repo kindly change it back to https://github.com/TeamMissKaur/MissKaur")
       sys.exit(1)
    else:
...
async def repo(client, message):
    ...
    await message.reply_photo(
        photo=ANON,
        caption=f"""**ʜᴇʏ​ {message.from_user.mention()},\n\nɪ ᴀᴍ [MissKaur](t.me/MissKaurXbot)**
**» ᴍʏ ᴅᴇᴠᴇʟᴏᴘᴇʀ​ :** [Team MissKaur](https://t.me/TeamMissKaur)
**» ᴩʏᴛʜᴏɴ ᴠᴇʀsɪᴏɴ :** `{y()}`
**» ʟɪʙʀᴀʀʏ ᴠᴇʀsɪᴏɴ :** `{o}` 
**» ᴛᴇʟᴇᴛʜᴏɴ ᴠᴇʀsɪᴏɴ :** `{s}` 
**» ᴘʏʀᴏɢʀᴀᴍ ᴠᴇʀsɪᴏɴ :** `{z}`
**MissKaur sᴏᴜʀᴄᴇ ɪs ɴᴏᴡ ᴩᴜʙʟɪᴄ ᴀɴᴅ ɴᴏᴡ ʏᴏᴜ ᴄᴀɴ ᴍᴀᴋᴇ ʏᴏᴜʀ ᴏᴡɴ ʙᴏᴛ.**
""",
        reply_markup=InlineKeyboardMarkup(
            [
                [
    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "MissKaur_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="MissKaur_fbanned_users.json",
        ...
        if args[0] == "csv":
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "MissKaur_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="MissKaur_fbanned_users.csv",
