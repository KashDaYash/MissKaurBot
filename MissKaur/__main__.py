...
import sys
import traceback
import MissKaur.modules.sql.users_sql as sql


from sys import argv
from typing import Optional
from MissKaur import (
    ALLOW_EXCL,
    CERT_PATH,
    DONATION_LINK,
    LOGGER,
...
from MissKaur.modules import ALL_MODULES
from MissKaur.modules.helper_funcs.chat_status import is_user_admin
from MissKaur.modules.helper_funcs.alternate import typing_action
from MissKaur.modules.helper_funcs.misc import paginate_modules
...
async def awake(event):
  MissKaur = event.sender.first_name
  MissKaur = "**♡ I,m MissKaur** \n\n"
  MissKaur += "**♡ I'm Working With Awesome Speed**\n\n"
  MissKaur += "**♡ MissKaur: LATEST Version**\n\n"
  MissKaur += "**♡ My Creator:** [TeamMissKaur](t.me/TeamMissKaur)\n\n"
  MissKaur += "**♡ Python-Telegram-Bot: v13.7**\n\n"
  MissKaur_BUTTON = [
      [
...
PM_START_TEXT = """
────「 [MissKaur](https://telegra.ph/file/7b8c52bd4c7b8b35ece20.jpg) 」────
*Hola! {},*
...
buttons = [
    [
                        InlineKeyboardButton(
                            ...
                            url="https://t.me/MissKaurXbot?startgroup=true")
                    ],
                   [
                       InlineKeyboardButton(text="☘️ ᴀʙᴏᴜᴛ ☘️", callback_data="MissKaur_"),
...
for module_name in ALL_MODULES:
    imported_module = importlib.import_module("MissKaur.modules." + module_name)
...
def MissKaur_all_callback(update, context):
    first_name = update.effective_user.first_name
    uptime = get_readable_time((time.time() - StartTime))
...
def MissKaur_about_callback(update, context):
    query = update.callback_query
    first_name = update.effective_user.first_name
    if query.data == "MissKaur_":
        query.message.edit_text(
            text=f"Hello dear {first_name}, My name is MissKaur. A Powerful Telegram Group Management Bot.\n My developer - @TeamMissKaur\n\nIf you have any Question, You can join our support chat. \nMy developer will help you. \nCheck Link Below",
            disable_web_page_preview=True,
            reply_markup=InlineKeyboardMarkup(
                [
                    [
...
def MissKaur_callback_data(update, context):
    query = update.callback_query
    uptime = get_readable_time((time.time() - StartTime))
    if query.data == "MissKaur_":
        query.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(
                [
                 [
                    InlineKeyboardButton(text="[► Back ◄]", callback_data="MissKaur_back")
                 ]
                ]
            ),
        )
    elif query.data == "MissKaur_back":
        first_name = update.effective_user.first_name
        query.message.edit_text(
                PM_START_TEXT.format(
                    escape_markdown(context.bot.first_name),
...
def main():
    ...
    all_modules_callback = CallbackQueryHandler(MissKaur_all_callback, pattern=r"all_cmds", run_async=True)
    MissKaur_about_handler = CallbackQueryHandler(MissKaur_about_callback, pattern=r"MissKaur_", run_async=True)
    data_callback_handler = CallbackQueryHandler(MissKaur_callback_data, pattern=r"MissKaur_", run_async=True)
    ...
    dispatcher.add_handler(MissKaur_about_handler)
    ...
    if WEBHOOK:
        ...
        LOGGER.info(f"MissKaur started, Using long polling. | BOT: [@{dispatcher.bot.username}]")
